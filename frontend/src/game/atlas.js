import "./bjs.js";
// game/atlas.js — 程序化 1024×1024 atlas，全城共用一张
// 4×4=16 格，每格 256×256
// 规范 §3.4 表格
...
const GRID = 4;
const CELL = ATLAS_SIZE / GRID; // 256
...
function rectForId(id) {
  ...
  const u1 = (col + 1) / GRID;
  const v1 = (row + 1) / GRID;
  return { u0, v0, u1, v1, x: col * CELL, y: row * CELL, w: CELL, h: CELL };
}
...
function drawWindowOld(ctx, x, y, w, h, rng) {
  ...
  for (let row = 0; row < h; row += floorH) {
    for (let col = 0; col < w; col += bayW) {
      ...
      ctx.fillRect(wx + 2, wy + 2, 7, 16);
      ctx.fillRect(wx + 11, wy + 2, 7, 16);
      ctx.fillStyle = "#5a3a1a";
      ctx.fillRect(wx + 9, wy + 2, 2, 16);
    }
  }
}
...
function drawSign(ctx, x, y, w, h, rng) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    ctx.fillRect(x + i * third, y, third, h);
    // 简单几何
    ctx.fillStyle = "#fff";
    if (i === 0) {
      ctx.beginPath();
      ...
      ctx.lineTo(x + i * third + third - 20, y + h - 40);
      ctx.closePath();
      ctx.fill();
    }
  }
...
function drawRoof(ctx, x, y, w, h, rng) {
  ...
  for (let k = 0; k < 20; k++) {
    ...
    ctx.fillRect(x + rng() * w, y + rng() * h, 20, 10);
  }
}
...
export function buildAtlasTexture(canvas) {
...
export function buildAtlas(scene) {
  ...
  if (typeof document !== "undefined") {
    ...
    canvas = new OffscreenCanvas(ATLAS_SIZE, ATLAS_SIZE);
  } else {
    // Node 环境无 canvas，返回 dummy
    return {
      texture: null,
      rect: getAtlasRect,
      canvas: null,
  ...
  buildAtlasTexture(canvas);

  let texture = null;
  if (scene) {
    try {
      // 使用 Babylon DynamicTexture
      // 尝试通过 scene 获取
      // 若 Babylon 已加载，DynamicTexture 会在 window.BABYLON
      if (typeof window !== "undefined" && window.BABYLON && window.BABYLON.DynamicTexture) {
        ...
        // 尝试 import @babylonjs/core 的 DynamicTexture
        // 由于我们在打包环境，DynamicTexture 已被包含，我们通过 scene 创建 RawTexture
        // 简化：直接创建 Texture from canvas via BABYLON.Texture
        const { DynamicTexture } = require ? require("@babylonjs/core/Materials/Textures/dynamicTexture") : {};
        if (DynamicTexture) {
          const dt = new DynamicTexture("cityAtlas", { width: ATLAS_SIZE, height: ATLAS_SIZE }, scene, false);
